...
class DistanceVector:
    ...
    def create_paths(self):
        ...
        for origin in range(0, self.network.size):
            self.shortest_paths[origin] = self.ask_neighbor(origin, self.shortest_paths[origin], origin)
    ...
    def ask_neighbor(self, origin, shortest_path, main_origin):
        for neighbor in self.network.adjacency[origin]:
        ...
        if self.INF in shortest_path:
            for neighbor in self.network.adjacency[origin]:
                shortest_path = self.ask_neighbor(neighbor, shortest_path, main_origin)
        return shortest_path
